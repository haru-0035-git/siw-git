...
class Pikachu(Pokemon):
    ...
    def attack_message(self, target):
        print(f'10万ボルト！{target.name}は{self._atk}ダメージを喰らった。{target.name}のパワーは{target.hp}だ。')
...
class Hitokage(Pokemon):
    ...
    def attack_message(self, target):
        print(f'ひのこ！{target.name}は{self._atk}ダメージを喰らった。{target.name}のパワーは{target.hp}だ。')